...
if _HAS_SOFTI2C and ssd1306:
    ...
    if ssd1306 is None:
        print("ERROR: ssd1306 module missing! OLED disabled.")
    elif not _HAS_SOFTI2C:
        print("ERROR: SoftI2C missing! OLED disabled.")
...
def display_transcript(text):
    ...
    for i, ln in enumerate(lines[:8]):
        oled.text(ln, 0, i * 8)
    print(f"DEBUG OLED LINES: {lines[:8]}")
    oled.show()
    ...
    while utime.ticks_diff(utime.ticks_ms(), start) < timeout:
        if esp_uart.any():
            ...
            if chunk:
                accumulated += chunk
                if b">" in accumulated:
                    gt = accumulated.find(b">")
                    return True, strip_at_echo(accumulated[:gt])
        ...
        if esp_uart.any():
            ...
            if chunk:
                ...
                for marker in (b"SEND OK", b"SEND FAIL", b"link is not valid"):
                    ...
                    if idx >= 0:
                        return marker == b"SEND OK", strip_at_echo(
                            accumulated[idx + len(marker):])
        time.sleep_ms(5)
...
def esp_send_data(link_id, data, timeout=8000):
    ...
    while offset < len(data):
        ...
        esp_uart.write(chunk)
        ok2, lv = _wait_send_ok(timeout)
        if lv:
            leftover += lv
        if not ok2:
            return False, leftover
            ...
            if is_recording:
                ...
                if n > 0:
                    ...
                    if stage_pos >= len(stage_buf):
                        ...
                        #     print(f"FORCING TEST UPDATE {audio_packets}")
                        #     display_transcript(f"TEST {audio_packets}")
