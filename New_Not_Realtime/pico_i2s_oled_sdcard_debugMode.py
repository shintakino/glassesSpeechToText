...
# CONFIGURATION
# ---------------------------------------------------------------
DEBUG_MODE    = True  # Enable to log output to SD card and show init status
...
def log_debug(msg):
    print(msg)
    if 'DEBUG_MODE' in globals() and DEBUG_MODE and sd_mounted:
        try:
            with open("/sd/debug.log", "a") as f:
                f.write(str(utime.ticks_ms()) + ": " + msg + "\n")
        except:
            pass
...
def display_status(line1, line2="", line3="", line4=""):
    msg = f"Display: {line1} | {line2} | {line3} | {line4}"
...
def init_i2s():
    ...
    try:
        ...
        if 'DEBUG_MODE' in globals() and DEBUG_MODE:
            log_debug(f"I2S Init Error: {e}")
        else:
            print(f"I2S Init Error: {e}")
        return False
...
def main():
    ...
    if DEBUG_MODE:
        log_debug("--- NEW BOOT ---")
        log_debug(f"SD Card Init: {sd_ok}")

    # Init OLED SECOND
    print("Init OLED...")
    oled_ok = init_oled()
    if DEBUG_MODE:
        log_debug(f"OLED Init: {oled_ok}")

    # Init I2S
    print("Init I2S...")
    i2s_ok = init_i2s()
    if DEBUG_MODE:
        log_debug(f"I2S Init: {i2s_ok}")

    if DEBUG_MODE:
        display_status("DEBUG MODE Init", 
                       f"SD: {'OK' if sd_ok else 'ERR'}", 
                       f"I2S: {'OK' if i2s_ok else 'ERR'}", 
                       f"OLED: {'OK' if oled_ok else 'ERR'}")
        if oled_ok:
            time.sleep(4)  # Hold so user can read status on PCB

    if not sd_ok:
        display_status("SD Card", "failed!", "Check wiring", "SCK=10 CS=13")
        while True:
            blink_led(2, 0.2)  # 2 blinks = SD error
            time.sleep(2)

    if not i2s_ok:
        display_status("I2S Mic", "failed!", "Check wiring")
        while True:
            blink_led(3, 0.2)  # 3 blinks = I2S error
            time.sleep(2)

    esp_ok = esp_init()
    if DEBUG_MODE:
        log_debug(f"ESP WiFi Init: {esp_ok}")
        display_status("DEBUG STATUS", f"WiFi: {'OK' if esp_ok else 'ERR'}")
        if oled_ok:
            time.sleep(2)
    ...
    if not esp_ok:
        ...
        while True:
            blink_led(4, 0.2)  # 4 blinks = ESP error
            time.sleep(2)
